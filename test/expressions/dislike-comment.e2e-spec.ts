...
describe('(POST) dislike comment', () => {
  ...
  async function dislikeComment(
    ...
    const result = await request(server)
      .post(
        EXPRESSIONS_ROUTE +
          ExpressionRoutes.DISLIKE_TO_COMMENT +
  ...
  describe('when dislikeComment is called', () => {
    ...
    describe.only('scenario : user leaves dislike expression on comment', () => {
      test('should return the created expression', async () => {
        const result = await dislikeComment(user.token);
    ...
    describe('scenario : user leaves again an expression on comment', () => {
      test('should return the created expression', async () => {
        const result = await dislikeComment(user.token, commentID);
